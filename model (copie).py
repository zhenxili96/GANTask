...
##
#    # LSGAN 
    self.d_loss_real = tf.reduce_sum(tf.square(self.D-1) ) # i left it jus because it s used in an other place but not used in the calcul of loss 
    self.d_loss_fake = tf.reduce_sum(tf.square(self.D_))# in equation there is no sum
    self.d_loss = (self.d_loss_real + self.d_loss_fake)
